function sendEmail() {
    ...
    xhr.onreadystatechange = function() {
        if (xhr.readyState == 4 && xhr.status == 200) {
            ...
            document.querySelector('.download-button').style.color = 'green';
            document.querySelector('.download-button').textContent = 'Запрос успешно отправлен';
        } else {
            document.querySelector('.download-button').style.color = 'red';
            document.querySelector('.download-button').textContent = 'Произошла ошибка при отправке запроса';
        }
        }
    };
